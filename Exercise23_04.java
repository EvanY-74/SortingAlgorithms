...
public class Exercise23_04 {
    ...
    // ---------------------------------------------------------------
    // Places the pivot element on the start
    // ---------------------------------------------------------------
    private static void placePivot(int[] list, int first, int last) {
        // edge case
        if (last - first < 2) return;
        int middle = (first + last) / 2;

        // Find the median of first, middle, last
        if ((list[first] < list[middle] && list[middle] < list[last]) || (list[last] < list[middle] && list[middle] < list[first])) {
            // System.out.println("Pivot: " + list[middle]);
            // swap low with middle
            int temp = list[middle];
            list[middle] = list[first];
            list[first] = temp;
        } else if ((list[middle] < list[first] && list[first] < list[last]) || (list[last] < list[first] && list[first] < list[middle])) {
            // System.out.println("Pivot: " + list[first]);
            return; // first is already at first position
        } else {
            // System.out.println("Pivot: " + list[last]);
